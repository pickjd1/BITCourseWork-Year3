...
//constructor
Sprite::Sprite(Graphics^ startCanvas, array<String^>^ startFileNames, int startNFrames, TileMap^ startTileMap)
{
	canvas = startCanvas;
...
Sprite::Sprite(Graphics^ startCanvas, array<String^>^ startFileNames, int startNFrames, int startXPos, int startYPos, int startDirection,  TileMap^ startTileMap)
{
	canvas = startCanvas;
	...
	void Sprite::ViewDraw(Rectangle^ viewRect)
	{
				
			int drawX = xPos - viewRect->X; 
			int drawY = yPos - viewRect->Y;

			Bitmap^ currentBitmap = spriteSheets[spriteDirection];

			Rectangle drawRect = Rectangle(frameWidth * currentFrame, 0, frameWidth, frameHeight);

			canvas->DrawImage(currentBitmap, drawX, drawY, drawRect, GraphicsUnit::Pixel);
